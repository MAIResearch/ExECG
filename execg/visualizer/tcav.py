...
def plot_tcav_ci(
    ...
    """Plot TCAV scores as grouped bar chart with CI for all concepts and layers.

    Creates a grouped bar chart where:
    - X-axis: Each concept
    - Each concept has a group of bars (one per layer)
    - CI (confidence interval) shown as error bars on top of each bar
    - Horizontal line at y=0.5 (baseline)
    - Y-axis: TCAV score
    ...
    Args:
        result_dict: Dictionary from TCAV.explain() method.
            Format: {concept_name: {layer_name: (mean_score, (ci_lower, ci_upper))}}
        figsize: Figure size. If None, auto-calculated based on data.
        colors: List of colors for each layer. If None, uses default colormap.
        save_file_path: Path to save the figure. If None, figure is not saved.
        dpi: DPI for saved figure. Default is 350.
    ...
    for i, layer in enumerate(layers):
        ...
        offset = (i - n_layers / 2 + 0.5) * bar_width
        ax.bar(
            x + offset,
            scores,
            bar_width,
    ...
    ax.axhline(
        y=0.5, color="red", linestyle="--", linewidth=1.5, label="Baseline (0.5)"
    )

    ax.set_xlabel("Concepts")
    ax.set_ylabel("TCAV Score")
    ax.set_title("TCAV Scores by Concept and Layer")
    ax.set_xticks(x)
    ax.set_xticklabels(concepts, rotation=45, ha="right")
    ax.set_ylim(0, 1)
